...
//        $pageManager->save($homepage);
    ...
    public function createTemoignagesPage(SiteInterface $site)
    {
        ...
        $page = $pageManager->create();
        $page->setSlug('/temoignages');
        $page->setUrl('/temoignages');
        $page->setName('Tous les témoignages');
        $page->setTitle('Tous les témoignages');
        $page->setEnabled(true);
        $page->setDecorate(1);
        $page->setRequestMethod('GET|POST|HEAD|DELETE|PUT');
        $page->setTemplateCode('default');
        ...
        // add the block
        $block->addChildren($temoignage = $blockManager->create());
        $temoignage->setType('sira.block.temoignage');
        $temoignage->setPosition(1);
        $temoignage->setEnabled(true);
        $temoignage->setPage($page);
    ...
    public function createTextSiraPage(SiteInterface $site, $url, $title, $directory, $menu, $parent, $color, $content)
    {
        ...
        $this->addReference($url, $page = $pageManager->create());
        $page->setSlug(sprintf('/%s', $url));
        $page->setUrl(sprintf('/%s', $url));
        $page->setName($title);
        $page->setTitle($title);
